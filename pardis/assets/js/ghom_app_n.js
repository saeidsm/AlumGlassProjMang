...
function openChecklistForm(fullElementId, elementType, dynamicContext) {
    ...
    .then((data) => {
      ...
      if (formElement) {
        ...
        function validateActiveStageWithPermissions(
          ...
          // --- VALIDATE CHECKLIST ITEMS (only if user can edit them) ---
          const itemRows = activeTab.querySelectorAll(".item-row");
          let checkedItems = 0;
          let totalItems = itemRows.length;

          if (canEditChecklistItems && totalItems > 0) {
            itemRows.forEach((itemRow, index) => {
              const itemText = itemRow
                .querySelector(".item-text")
                .textContent.trim();
              const radio = itemRow.querySelector(
              ...
              if (radio) {
                ...
                if (
                  ...
                  warnings.push(
                    `آیتم "${itemText}" رد شده اما توضیحی ارائه نشده است`
                  );
                }
              } else {
                itemRow.classList.add("incomplete");
                // Only warn about unchecked items if user can actually edit them
                warnings.push(
                  `آیتم شماره ${index + 1}: "${itemText}" انتخاب نشده است`
                );
              }
            });
          ...
          // --- VALIDATE CONSULTANT SECTION (only if user can edit it) ---
          const consultantSection = activeTab.querySelector(
            ".consultant-section"
          );
          if (consultantSection && canEditConsultantSection) {
            ...
            // For superuser, minimal validation: only require date and status if overall_status is filled
            if (isSuperuser) {
              // Only check if overall_status is filled - then date becomes required
              if (overallStatus && overallStatus.value.trim()) {
                hasData = true;
                info.push(`وضعیت کلی: ${overallStatus.value}`);

                // If status is filled, date is required
                if (!inspectionDate || !inspectionDate.value.trim()) {
                  errors.push(
                    "اگر وضعیت کلی را انتخاب کردید، تاریخ بازرسی الزامی است"
                  );
                }
              }

              // Always mark data if fields are filled
              if (inspectionDate && inspectionDate.value.trim()) {
                hasData = true;
              ...
              // For consultants, these fields are required when it's their turn
              if (!overallStatus || !overallStatus.value.trim()) {
            ...
            // Check file uploads (always count as data if present)
            const fileInputs =
            ...
            fileInputs.forEach((input) => {
              if (input.files && input.files.length > 0) {
                hasData = true;
                info.push(
                  `${input.files.length} فایل جدید برای آپلود انتخاب شده`
                );
              }
            });
          }

          // --- VALIDATE CONTRACTOR SECTION (only if user can edit it) ---
          const contractorSection = activeTab.querySelector(
            ".contractor-section"
          );
          if (contractorSection && canEditContractorSection) {
            ...
            // For superuser, completely free - no requirements at all
            if (isSuperuser) {
              // Just mark data if any field is filled, no validation requirements
              ...
              // For contractors, these fields are required when it's their turn
              if (!contractorStatus || !contractorStatus.value.trim()) {
            ...
            // Check contractor file uploads (always count as data if present)
          ...
          // --- OVERALL DATA CHECK ---
          // For superuser: completely flexible - just need some data somewhere
          // For other roles: they must complete their designated sections
          if (!hasData) {
            if (isSuperuser) {
              errors.push(
                "حداقل یک فیلد باید پر شود. شما آزاد هستید هر بخشی را که می‌خواهید ویرایش کنید."
              );
            } else if (canEditConsultantSection && !canEditContractorSection) {
              errors.push("بخش مشاور باید تکمیل شود.");
            } else if (canEditContractorSection && !canEditConsultantSection) {
              errors.push("بخش پیمانکار باید تکمیل شود.");
            } else if (
              ...
              errors.push("هیچ داده‌ای برای ذخیره یافت نشد.");
            }
          }

          // Add permission info for clarity
          const permissionsInfo = [];
          if (canEditChecklistItems) permissionsInfo.push("چک لیست");
          if (canEditConsultantSection) {
            if (isSuperuser) {
              permissionsInfo.push(
                "بخش مشاور (آزاد، فقط اگر وضعیت انتخاب شد تاریخ لازم است)"
              );
            } else {
              permissionsInfo.push("بخش مشاور");
            }
          }
          if (canEditContractorSection) {
            if (isSuperuser) {
              permissionsInfo.push("بخش پیمانکار (آزاد)");
            } else {
              permissionsInfo.push("بخش پیمانکار");
            }
          }

          if (permissionsInfo.length > 0) {
            info.unshift(`مجوز ویرایش: ${permissionsInfo.join("، ")}`);
          }
        ...
        // 3. ADD THIS HELPER FUNCTION FOR HIGHLIGHTING ERRORS
        ...
        // 4. ENHANCED CONFIRMATION MODAL (OPTIONAL IMPROVEMENT)
        // You can also enhance your showConfirmationModal function to show permission information:

        function showConfirmationModalEnhanced(stageData, validationResult) {
          const modal = document.createElement("div");
          modal.className = "confirmation-modal-overlay";
          modal.style.cssText = `
    position: fixed;
  ...
  `;
          ...
          if (validationResult.permissions) {
            const permissions = [];
            if (validationResult.permissions.canEditChecklistItems)
              permissions.push("چک لیست");
            if (validationResult.permissions.canEditConsultantSection)
              permissions.push("بخش مشاور");
            if (validationResult.permissions.canEditContractorSection)
              permissions.push("بخش پیمانکار");

            if (permissions.length > 0) {
              permissionHTML = `
        <div style="background: #e3f2fd; border: 1px solid #2196f3; padding: 10px; border-radius: 5px; margin: 10px 0;">
          <strong>مجوزهای ویرایش شما:</strong> ${permissions.join("، ")}
        </div>
      `;
            }
          }

          // Build items HTML only if user had permission to edit checklist
          let itemsHTML = "";
          if (
            stageData.items &&
            stageData.items.length > 0 &&
            validationResult.permissions?.canEditChecklistItems
          ) {
            itemsHTML = `
      <h4>آیتم‌های چک لیست:</h4>
      <ul style="margin: 10px 0; padding-right: 20px;">
    ...
    `;
          ...
          // Build consultant HTML only if user had permission to edit consultant section
          let consultantHTML = "";
          if (
            validationResult.permissions?.canEditConsultantSection &&
            (stageData.overall_status ||
              stageData.inspection_date ||
              stageData.notes)
          ) {
    ...
    `;
          ...
          // Build contractor HTML only if user had permission to edit contractor section
          let contractorHTML = "";
          if (
            validationResult.permissions?.canEditContractorSection &&
            (stageData.contractor_status ||
              stageData.contractor_date ||
              stageData.contractor_notes)
          ) {
    ...
    `;
          ...
          modalContent.innerHTML = `
    <h3 style="margin: 0 0 20px 0; color: #2c3e50;">تایید نهایی - ${
    ...
    <div style="background: #f8f9fa; padding: 15px; border-radius: 5px; margin: 15px 0;">
      ...
      ${
        !itemsHTML && !consultantHTML && !contractorHTML
          ? "<p>هیچ داده جدیدی برای ذخیره وجود ندارد.</p>"
          : ""
      }
    </div>
