<?php
// /public_html/pardis/api/get_element_data.php (MULTI-STAGE SUPPORT)
...
try {
    ...
    if (!empty($stages_history)) {
        if (in_array($user_role, ['admin', 'superuser', 'supervisor','planner'])) {
        ...
        $can_user_edit = in_array($user_role, ['admin', 'superuser','supervisor','planner']);
    }
