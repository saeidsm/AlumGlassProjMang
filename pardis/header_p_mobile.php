...
if ($currentProjectConfigKeyInSession !== 'pardis') { // Hardcoded for this Fereshteh header
...
switch ($role) {
    ...
    case 'car':
        ...
        break;
    case 'coa':
        $roleText = ' عمران آذرستان';
        break;
    case 'crs':
        $roleText = ' شرکت ساختمانی رس';
        break;
                case 'cod':
        $roleText = 'شرکت طرح و نقش آدرم';
        break;
}
...
<head>
    ...
    <title>دانشگاه خاتم - <?php echo htmlspecialchars($pageTitle); ?></title>
  ...
  .user-avatar-common {
        ...
        .mobile-header-weather-widget .weather-icon {
            font-size: 1.4em;
        }
        .mobile-header-weather-widget .weather-temp {
            font-weight: 600;
        }
        .mobile-header-weather-widget .weather-loading {
            font-size: 0.75rem;
        }
         <?php
        // Add any additional styles specific to a page.
...
<body>
    <div class="main-container">
        <header class="top-header">
            <div class="header-content">
                <div class="site-logo-container">
                    <a href="/">
                        <img src="/assets/images/alumglass-farsi-logo-H40.png" alt="alumglass Logo" class="site-logo">
                    </a>
                ...
                <div id="mobile-header-weather-container" style="width: 100%; order: 3;"></div>
        ...
        <nav class="nav-container">
            <div class="navigation" id="main-nav">
                <?php if (hasAccess(['admin', 'superuser', 'user','supervisor', 'planner','cod','car'])): ?>
                ...
                <?php endif; ?>
                  <?php if (hasAccess(['admin', 'superuser', 'user','supervisor', 'planner'])): ?>
                        <a href="daily_reports.php"class="nav-item<?php echo getActiveClass('daily_reports.php', $current_page_filename); ?>">
                    ...
                    <?php endif; ?>
                    <?php if (hasAccess(['admin', 'superuser', 'user','supervisor', 'planner','cod','car'])): ?>
                        <a href="daily_reports_dashboard_ps.php"class="nav-item<?php echo getActiveClass('daily_reports_dashboard_ps.php', $current_page_filename); ?>">
                            <svg class="nav-icon" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M6.94028 2C7.35614 2 7.69326 2.32421 7.69326 2.72414V4.18487C8.36117 4.17241 9.10983 4.17241 9.95219 4.17241H13.9681C14.8104 4.17241 15.5591 4.17241 16.227 4.18487V2.72414C16.227 2.32421 16.5641 2 16.98 2C17.3958 2 17.733 2.32421 17.733 2.72414V4.24894C19.178 4.36022 20.1267 4.63333 20.8236 5.30359C21.5206 5.97385 21.8046 6.88616 21.9203 8.27586L22 9H2.92456H2V8.27586C2.11571 6.88616 2.3997 5.97385 3.09665 5.30359C3.79361 4.63333 4.74226 4.36022 6.1873 4.24894V2.72414C6.1873 2.32421 6.52442 2 6.94028 2Z" />
                                <path opacity="0.5" d="M21.9995 14.0001V12.0001C21.9995 11.161 21.9963 9.66527 21.9834 9H2.00917C1.99626 9.66527 1.99953 11.161 1.99953 12.0001V14.0001C1.99953 17.7713 1.99953 19.6569 3.1711 20.8285C4.34267 22.0001 6.22829 22.0001 9.99953 22.0001H13.9995C17.7708 22.0001 19.6564 22.0001 20.828 20.8285C21.9995 19.6569 21.9995 17.7713 21.9995 14.0001Z" />
                            </svg>
                    ...
                    <?php endif; ?>
                                 ...
                                 <a href="forms_list.php"class="nav-item<?php echo getActiveClass('forms_list.php', $current_page_filename); ?>">
                                    صفحه فرم‌ها
                                </a>
                     <a href="zirsazi_status.php"class="nav-item<?php echo getActiveClass('zirsazi_status.php', $current_page_filename); ?>">
                                    متریال زیرسازی
                                </a>
                                  <a href="packing_list_viewer.php"class="nav-item<?php echo getActiveClass('packing_list_viewer.php', $current_page_filename); ?>">
                                    متریال پروفیل آلومنیومی
                                </a>
                                <?php if (hasAccess(['admin', 'superuser','planner','supervisor'])): ?>
                                    <a href="materials_log.php"class="nav-item<?php echo getActiveClass('materials_log.php', $current_page_filename); ?>">
                                        مدیریت ورود مصالح
                                    </a>
                                <?php endif; ?>
                                <?php if (hasAccess(['admin', 'superuser','planner','supervisor'])): ?>
                                    <a href="viewer_3d_mobile.php"class="nav-item<?php echo getActiveClass('viewer_3d_mobile.php', $current_page_filename); ?>">
    🏗️ نمایشگر مدل‌های سه‌بعدی
                                    </a>
                                <?php endif; ?>
                <?php if (hasAccess(['admin', 'superuser', 'user', 'cat', 'car', 'cod', 'crs'])): ?>
  ...
  <?php if (hasAccess(['admin', 'superuser'])): ?>
                    <a href="telegram_report_manual.php" target="_blank" rel="noopener noreferrer" class="nav-item<?php echo getActiveClass('telegram_report_manual.php', $current_page_filename); ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <circle cx="12" cy="12" r="10"></circle>
                            <path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path>
                            <line x1="12" y1="17" x2="12.01" y2="17"></line>
                        </svg>
ارسال دستی گزارشات تلگرام                    </a>
                <?php endif; ?>
                <?php // --- Messages Link Block ---
                // Original access check
                if (hasAccess(['admin', 'superuser', 'supervisor', 'planner', 'user', 'receiver', 'cnc_operator', 'guest', 'car', 'cod',])) : // Adjust roles as needed
                ?>
                    <a href="/messages.php"
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
             ...
             document.addEventListener("DOMContentLoaded", () => {
            // Ensure the class is loaded and we haven't already created this instance
            if (typeof WeatherIntegration !== 'undefined' && !window.mobileWeatherIntegration) {
                
                // Custom function to display weather in a compact mobile format
                function displayMobileHeaderWeather(weather) {
                    const displayElement = document.getElementById("mobile-header-weather-container");
                    if (!displayElement) return;

                    displayElement.innerHTML = `
                        <div class="mobile-header-weather-widget">
                ...
                // Custom function for loading/error states
                function displayMobileHeaderLoadingError(message) {
                    const displayElement = document.getElementById("mobile-header-weather-container");
                    if (!displayElement) return;
                    displayElement.innerHTML = `
                        <div class="mobile-header-weather-widget">
                            <div class="weather-loading">${message}</div>
                        </div>
                    `;
                }

                // Configuration for the mobile widget
                const mobileWeatherConfig = {
                    apiEndpoint: '/pardis/weather_api.php',
                    weatherDisplayId: 'mobile-header-weather-container',
                    weatherSelectId: null, // No form select to update
                    autoFetch: false,      // We will fetch manually
                    location: 'pardis'     // Hardcode to pardis location
                };

                // Create a unique instance to avoid conflicts with other pages/headers
                window.mobileWeatherIntegration = new WeatherIntegration(mobileWeatherConfig);
                
                // Override the default class methods with our custom ones
                window.mobileWeatherIntegration.displayWeather = displayMobileHeaderWeather;
                
                window.mobileWeatherIntegration.fetchWeather = async function(location = null) {
                    displayMobileHeaderLoadingError('...');
                    try {
                        const url = `${this.apiEndpoint}?location=${this.location}`;
                        const response = await fetch(url);
                        const result = await response.json();
                        if (!result.success) { throw new Error(result.message); }
                        this.displayWeather(result.data);
                    } catch (error) {
                        console.error("Mobile weather fetch error:", error);
                        displayMobileHeaderLoadingError('خطا');
                    }
                };
                
                // Manually trigger the first fetch
                window.mobileWeatherIntegration.fetchWeather();
            }
        });
        </script>