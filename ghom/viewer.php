...
<head>
    ...
    <link rel="stylesheet" href="/ghom/assets/css/viewer.css">
</head>
  ...
  <div id="stage-filter-panel">
    ...
    <div class="panel-footer">
      ...
      <button id="reset-view-btn" class="btn btn-back">نمایش وضعیت کلی</button>
        ...
        // Define which layers can be toggled
        const layerIds = ['Curtainwall', 'GLASS', 'GFRC']; // Add any other layers here
        let layerVisibility = {};

        layerIds.forEach(layerId => {
            const groupElement = svgElement.getElementById(layerId);
            if (groupElement) { // Only create a button if the layer exists
        ...
        // Add a single smart event listener for all layer buttons
        controlsContainer.addEventListener('click', (e) => {
            ...
            // Toggle visibility for the clicked layer
            ...
            if (curtainwallLayer && glassLayer && glassButton) {
                ...
                if (isCurtainwallVisible) {
                    ...
                    glassButton.disabled = true; // Disable the button
                    layerVisibility['GLASS'] = false; // Update state
                } else {
                    // If Curtainwall is OFF, re-enable the Glass button
...
function renderElementColors(elementsToRender, filterStatus = 'all') {
    ...
    let activeTooltipElement = null; // Track currently active tooltip for mobile
    ...
    svgElement.querySelectorAll('path[id], rect[id]').forEach(el => {
        if (elementDataMap.has(el.id)) {
            ...
            if (filterStatus === 'all' || status === filterStatus) {
                ...
                // Function to show tooltip with element data
                ...
                if (isTouchDevice) {
                    ...
                    el.addEventListener('touchstart', (e) => {
                        ...
                        // If this element is already active, hide tooltip
                        if (activeTooltipElement === el) {
                            hideElementTooltip();
                            return;
                        }
                    ...
                    // Optional: Also handle touch end for better UX
                ...
                // Click event for both touch and mouse (for navigation or additional actions)
                el.addEventListener('click', (e) => {
                    e.stopPropagation(); // Prevent event bubbling
                    console.log(`Element clicked: ${element.element_id}`);
                    
                    // You can add navigation or other click actions here
                    // For example, opening a detailed view:
                    // window.location.href = `/ghom/element_details.php?id=${element.element_id}`;
                });
            }
        }
    });
