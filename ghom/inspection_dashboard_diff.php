...
try {
    ...
    // This query aggregates the latest status information for each element.
    ...
    FROM elements e
    LEFT JOIN inspections i ON i.element_id = e.element_id
...
<body data-user-role="<?php echo escapeHtml($_SESSION['role']); ?>">
    <div class="dashboard-container">
        ...
        <!-- Section to open plans with statuses -->
        <div class="table-container" style="margin-bottom: 20px; background-color: #e9f5ff;">
            ...
            <div class="filters">
                ...
                <div class="form-group">
                    ...
                    <button type="button" class="btn report-btn" data-status="Pending" style="background-color: #6c757d;">در انتظار</button>
                    <button type="button" class="btn report-btn" data-status="all" style="background-color: #17a2b8;">همه وضعیت‌ها</button>
                </div>
            </div>
        </div>
            ...
            <?php foreach ($data_by_type as $type => $elements_data): ?>
                <div id="tab-<?php echo escapeHtml($type); ?>" class="tab-content table-container">
                    ...
                    <div class="column-filters">
                        <div class="form-group"><label>فیلتر کد المان:</label><input type="text" class="filter-input" data-column="element_id" placeholder="فیلتر..."></div>
                        <div class="form-group"><label>فیلتر فایل نقشه:</label><select class="filter-select" data-column="plan_file">
                        ...
                        <div class="form-group"><label>فیلتر وضعیت:</label><select class="filter-select" data-column="status">
                        ...
                        <tbody>
                            <?php foreach ($elements_data as $element_id => $row): ?>
                                <tr class="data-row <?php echo get_status_class($row); ?>"
                                    ...
                                    <!-- NEW: Inspection Date data cell -->
                                    <td><?php echo $row['inspection_date'] ? jdate('Y/m/d', strtotime($row['inspection_date'])) : '---'; ?></td>
                                    <td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize date pickers on the main page
            ...
            function setFormPermissions(formEl, buttonsEl, role) {
                ...
                if (role === 'admin') {
                    ...
                    contractorSection.disabled = false; // Admin can edit everything
            ...
            async function openChecklistForm(elementId, elementType, inspectionData) {
                try {
                    // Fetch the latest checklist data for the element
                ...
                <form id="checklist-form" novalidate>
                    ...
                    // Re-initialize datepicker for the newly created form inputs
                    jalaliDatepicker.startWatch({
            ...
            async function handleFormSubmit(event) {
                ...
                try {
                    ...
                    // Check if response is ok, otherwise read the text for server-side errors
                    if (!response.ok) {
                    ...
                    if (result.status === 'success') {
                        alert(result.message); // Replace with a nicer modal/toast later
                        hideFormPanel();
                        location.reload();
                    } else {
            ...
            // --- JDF Date Helper Functions (already in your code, keeping them) ---
            function jdate(format, timestamp) {
                if (!timestamp) return '';
                let date = new Date(timestamp * 1000);
                // This function now correctly uses Intl.DateTimeFormat for robust localization.
                // The 'format' parameter is kept for compatibility but is not used by this new implementation.
            ...
            // --- FIXED: Event listening for showing/hiding the form panel ---
            // Listen on the whole document for clicks to open the form
            ...
            // Listen specifically on the overlay to close the form.
            // This prevents clicks inside the datepicker from being caught.
            formOverlay.addEventListener('click', hideFormPanel);


            // Plan view report buttons
