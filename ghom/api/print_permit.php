...
// 2. Load Config (JSON) - Tries file first, falls back to inline JS below
$possible_paths = [
...
// Ensure SVG filename has extension
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .section-title {
            ...
            padding: 5px 10px; font-weight: bold; margin: 15px 0 10px 0;
        ...
        .signature-label {
            ...
            white-space: pre-wrap; /* Allows text wrap */
        }
...
<body>
    ...
    <div class="no-print">
        ...
        <div style="margin-top:10px; color:red; font-size:12px;">
            * لطفا این فرم را چاپ کرده، امضا نمایید و سپس تصویر آن را در کارتابل آپلود کنید.
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            try {
                ...
                // 3. Set Initial Contractor
                const contractorSelect = document.getElementById('display-contractor');
                
                // Logic to set select value: Try to find match, otherwise add option
                let matchFound = false;
                for (let i = 0; i < contractorSelect.options.length; i++) {
                    if (contractorSelect.options[i].value === foundContractor) {
                ...
                if (!matchFound && foundContractor) {
                    const opt = document.createElement("option");
                    opt.value = foundContractor;
                    opt.text = foundContractor;
                    contractorSelect.add(opt);
                    contractorSelect.value = foundContractor;
                }

                // 4. Trigger Update (Sets initial Note and Sig)
