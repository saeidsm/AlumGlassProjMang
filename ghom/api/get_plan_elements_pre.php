<?php
// /ghom/api/get_plan_elements_pre.php (DEFINITIVELY CORRECTED)
...
try {
    ...
    // Your robust query, now with the `permit_file` added.
    $sql = "
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        ...
        $elements_with_data[$element_id] = [
            ...
            'floor'      => $row['floor_level'] ?? $row['floor'],
            'axis'       => $row['axis_span'] ?? $row['axis'],
            'contractor' => $row['contractor'],
            'block'      => $row['block'],
            'zoneName'   => $row['zone_name'],
            'permit_file' => $row['permit_file'] ?? null // <-- ADDED THIS TO THE FINAL OUTPUT
        ];
    }
